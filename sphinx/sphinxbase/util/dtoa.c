...
typedef union { APPROX double d; ULong L[2]; } U;
...
d2b
#ifdef KR_headers
	(d, e, bits) APPROX double d; int *e, *bits;
#else
	(APPROX double _d, int *e, int *bits)
#endif
{
	Bigint *b;
	int de, k;
...
#endif
{
 ...
 drop_down:
		...
		if ((ENDORSE(aadj = ratio(delta, bs))) <= 2.) {
			if (dsign)
				aadj = dval(aadj1) = 1.;
...
#ifdef Avoid_Underflow
			if (scale && y <= 2*P*Exp_msk1) {
				if ((ENDORSE(aadj)) <= 0x7fffffff) {
					if ((z = (uint32)(ENDORSE(aadj))) <= 0)
						z = 1;
					aadj = z;
					dval(aadj1) = dsign ? aadj : -aadj;
					}
		...
		if (y == z) {
			...
			if (dsign || word1(rv) || word0(rv) & Bndry_mask) {
				if ((ENDORSE(aadj)) < .4999999 || (ENDORSE(aadj)) > .5000001)
					break;
				}
			else if ((ENDORSE(aadj)) < .4999999/FLT_RADIX)
				break;
			}
#endif
 cont:
