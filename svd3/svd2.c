...
static APPROX float maxarg1,maxarg2;
#define FMAX(a,b) (maxarg1 = (a),maxarg2 = (b),(maxarg1) > (maxarg2) ? (maxarg1) : (maxarg2))

static int iminarg1,iminarg2;
#define IMIN(a,b) (iminarg1 = (a),iminarg2 = (b),(iminarg1 < (iminarg2) ? (iminarg1) : iminarg2))

static APPROX float sqrarg;
#define SQR(a) ((sqrarg = (a)) == 0.0 ? 0.0 : sqrarg * sqrarg)


// calculates sqrt( a^2 + b^2 ) with decent precision
float pythag(APPROX float a, APPROX float b);
float pythag(APPROX float a, APPROX float b) {
  float absa,absb;
...
*/
int svd2(APPROX float **a, int nRows, int nCols, APPROX float *w, APPROX float **v) {
  int flag,i,its,j,jj,k,l,nm;
  APPROX float anorm,c,scale, x, y, z, g, f, h, s, *rv1;
  ...
  for(i=0;i<nCols;i++) {
    ...
    g = s = scale = 0.0;
    if(i < nRows) {
      for(k=i;k<nRows;k++) scale += fabs(ENDORSE(a[k][i]));
      if(ENDORSE(scale)) {
	for(k=i;k<nRows;k++) {
	  a[k][i] /= scale;
	  s += a[k][i] * a[k][i];
    ...
    if(i < nRows && i != nCols-1) {
      for(k=l;k<nCols;k++) scale += fabs(ENDORSE(a[i][k]));
      if(ENDORSE(scale))  {
	for(k=l;k<nCols;k++) {
	  a[i][k] /= scale;
	  s += a[i][k] * a[i][k];
    ...
    anorm = FMAX(anorm, (fabs(ENDORSE(w[i])) + fabs(temp)));
  }
  ...
  for(k=nCols-1;k>=0;k--) {
	...
	if((fabs(ENDORSE(w[nm])) + ENDORSE(anorm)) == ENDORSE(anorm)) break;
      }
      if(flag) {
	c = 0.0;
	s = 1.0;
	for(i=l;i<=k;i++) {
	  f = s * rv1[i];
	  rv1[i] = c * rv1[i];
	  if((fabs(ENDORSE(f)) + ENDORSE(anorm)) == ENDORSE(anorm)) break;
	  g = w[i];
	  h = pythag(f,g);
	  w[i] = h;
	  h = 1.0 / h;
