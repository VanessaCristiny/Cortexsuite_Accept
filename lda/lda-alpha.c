...
{ return(D * (lgamma((ENDORSE(K * a))) - K * lgamma((ENDORSE(a)))) + (a - 1) * ss); }
...
APPROX double opt_alpha(APPROX double ss, int D, int K)
{
    APPROX double a, log_a, init_a = 100;
    APPROX double f, df, d2f;
    int iter = 0;
    FILE *fp;
    ...
    while ((fabs((ENDORSE(df))) > NEWTON_THRESH) && (iter < MAX_ALPHA_ITER));
    return(exp(log_a));
}
