...
void run_em(char* start, char* directory, corpus* corpus)
{
    ...
    int i = 0;
    APPROX double likelihood, likelihood_old = 0, converged = 1;
    sprintf(filename, "%s/likelihood.dat", directory);
    FILE* likelihood_file = fopen(filename, "w");

    while ((((ENDORSE(converged < 0))) || ((ENDORSE(converged)) > (ENDORSE(EM_CONVERGED))) || (i <= 2)) && (i <= (ENDORSE(EM_MAX_ITER))))
    {
        ...
        if ((ENDORSE(converged)) < 0) VAR_MAX_ITER = VAR_MAX_ITER * 2;
        ...
        if ((i % LAG) == 0)
        {
            ...
            save_gamma(filename, (ENDORSE(var_gamma)), corpus->num_docs, model->num_topics);
        }
    }
...
void infer(char* model_root, char* save, corpus* corpus)
{
	...
	likelihood = ENDORSE(lda_inference(doc, model, var_gamma[d], phi));
